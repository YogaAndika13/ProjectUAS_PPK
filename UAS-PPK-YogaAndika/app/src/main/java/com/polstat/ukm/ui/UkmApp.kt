...
fun UkmApp(
    ...
    val showFloatingActionButton = when (navBackStackEntry?.destination?.route) {
        UkmScreen.UkmManagement.name -> true
        else -> false
    }
    val floatingActionButtonDestination = when (navBackStackEntry?.destination?.route) {
        UkmScreen.UkmManagement.name -> UkmScreen.CreateUkm.name
        else -> ""
    }
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            ...
            NavHost(
                ...
                composable(route = UkmScreen.Login.name) {
                    LoginScreen(
                        ...
                        showMessage = { title, body ->
                            ukmAppViewModel.showMessageDialog(
                                title,
                                body
                            )
                        }
                ...
                composable(route = UkmScreen.UkmManagement.name) {
                    UkmManagementScreen(
                        showMessage = { title, body ->
                            ukmAppViewModel.showMessageDialog(
                                title,
                ...
                composable(route = UkmScreen.CreateUkm.name) {
                    CreateUkmScreen(
                        showMessage = { title, body ->
                            ukmAppViewModel.showMessageDialog(
                                title,
